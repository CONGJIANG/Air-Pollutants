...
p_no2 <- ggplot(no2_trend, aes(x = month, y = mean_val, color = factor(year), group = year)) +
  ...
  labs(title = "NO2", x = "Month", y = "NO2 (µg/m³)", color = "Year") +
...
# NO Trend
# Convert NO from ppb to μg/m³
data <- data |>
  mutate(mean_NOug = mean_NOppb * 1.23)

no_trend <- data |>
  group_by(year, month) |>
  summarize(mean_val = mean(mean_NOug, na.rm = TRUE), .groups = "drop") |>
  arrange(year, month) |>
  convert_months()
...
p_no <- ggplot(no_trend, aes(x = month, y = mean_val, color = factor(year), group = year)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.2, alpha = 0.6) +
  labs(title = "NO", x = "Month", y = "NO (µg/m³)", color = "Year") +
  theme_minimal(base_size = 10) +
  theme(plot.title = element_text(face = "bold", size = 11),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
...
p_so2 <- ggplot(so2_trend, aes(x = month, y = mean_val, color = factor(year), group = year)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.2, alpha = 0.6) +
  labs(title = "SO2", x = "Month", y = "SO2 (µg/m³)", color = "Year") +
  theme_minimal(base_size = 10) +
  theme(plot.title = element_text(face = "bold", size = 11),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
...
p_ox <- ggplot(ox_trend, aes(x = month, y = mean_val, color = factor(year), group = year)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.2, alpha = 0.6) +
  labs(title = "Ox", x = "Month", y = "Ox (µg/m³)", color = "Year") +
  theme_minimal(base_size = 10) +
  theme(plot.title = element_text(face = "bold", size = 11),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
...
p_o3 <- ggplot(o3_trend, aes(x = month, y = mean_val, color = factor(year), group = year)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.2, alpha = 0.6) +
  labs(title = "O3", x = "Month", y = "O3 (µg/m³)", color = "Year") +
  theme_minimal(base_size = 10) +
  theme(plot.title = element_text(face = "bold", size = 11),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
...
mo_trend <- data |>
  group_by(year, month) |>
  summarize(mean_val = mean(AggregatedDeath, na.rm = TRUE), .groups = "drop") |>
  arrange(year, month) |>
  convert_months()

# MO trend by year (monthly progression within each year)
p_mo <- ggplot(mo_trend, aes(x = month, y = mean_val, color = factor(year), group = year)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.2, alpha = 0.6) +
  labs(title = "Mortality", x = "Month", y = "Mortality", color = "Year") +
  theme_minimal(base_size = 10) +
  theme(plot.title = element_text(face = "bold", size = 11),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 8))

# Aggregate MO by year (sum of monthly values)
mo_by_year <- mo_trend |>
  group_by(year) |>
  summarize(
    total_co = sum(mean_val, na.rm = TRUE),
...
# Combine all plots in one figure
combined_trend_plot <- (p_no2 + p_no)/  (p_ox + p_so2) / (p_pm25 + p_pm10) +  (p_co + p_o3) +
  plot_annotation(title = "Trend of 12-Month Moving Average Concentrations for All Pollutants",
                  theme = theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5)))


combined_trend_plot <- (p_no2 + p_no + p_so2 + p_ox) / (p_pm25 + p_pm10 + p_o3 + p_co) / p_mo +
  plot_annotation(title = "Trend of 12-Month Moving Average Concentrations for All Pollutants and Mortality",
                  theme = theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5)))
combined_trend_plot
...
p_mo
####################
### Annual Means and Standard Deviations for All Pollutants
####################

pollutant_list <- list(
  NO2 = "mean_NO2_ensemble",
  NO = "mean_NOug",
  SO2 = "mean_SO2_ensemble",
  Ox = "mean_Ox",
  PM25 = "mean_PM25_ensemble",
  PM10 = "mean_PM10ug",
  O3 = "mean_O3_pred_cal",
  CO = "CO_pred"
)
...
annual_stats <- data |>
  group_by(year) |>
  summarize(
    NO2_mean = mean(mean_NO2_ensemble, na.rm = TRUE),
    NO2_sd = sd(mean_NO2_ensemble, na.rm = TRUE),
    NO_mean = mean(mean_NOug, na.rm = TRUE),
    NO_sd = sd(mean_NOug, na.rm = TRUE),
    SO2_mean = mean(mean_SO2_ensemble, na.rm = TRUE),
    SO2_sd = sd(mean_SO2_ensemble, na.rm = TRUE),
    Ox_mean = mean(mean_Ox, na.rm = TRUE),
    Ox_sd = sd(mean_Ox, na.rm = TRUE),
    PM25_mean = mean(mean_PM25_ensemble, na.rm = TRUE),
    PM25_sd = sd(mean_PM25_ensemble, na.rm = TRUE),
    PM10_mean = mean(mean_PM10ug, na.rm = TRUE),
    PM10_sd = sd(mean_PM10ug, na.rm = TRUE),
    O3_mean = mean(mean_O3_pred_cal, na.rm = TRUE),
    O3_sd = sd(mean_O3_pred_cal, na.rm = TRUE),
    CO_mean = mean(CO_pred, na.rm = TRUE),
    CO_sd = sd(CO_pred, na.rm = TRUE),
    Mortality_mean = mean(AggregatedDeath, na.rm = TRUE),
    Mortality_sd = sd(AggregatedDeath, na.rm = TRUE),
    .groups = "drop"
  )
